...
async def get_vast_xml(
    ...
    Used when serving ads via VAST tag URL.
    ...
    # Parse ad_id
    parts = ad_id.split("_")
    campaign_id = parts[1] if len(parts) >= 2 else "0"
...
<VAST version="{vast_version}">
  <Ad id="{ad_id}">
    <InLine>
      ...
      <AdTitle>Video Ad {ad_id}</AdTitle>
      ...
      <Creatives>
        <Creative id="{creative_id}">
          <Linear>
            ...
            <VideoClicks>
              <ClickThrough><![CDATA[{tracking.click_url}]]></ClickThrough>
            </VideoClicks>
            <MediaFiles>
              <MediaFile delivery="progressive" type="video/mp4" width="1920" height="1080">
                <![CDATA[{base_url}/static/video/{creative_id}.mp4]]>
              </MediaFile>
            </MediaFiles>
          </Linear>
        </Creative>
